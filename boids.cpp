...
// calculating mean distance and its RMS for a given time
Vector Flock::get_distance_mean_RMS() const {
  ...
  if (this->size() > 1) {
    ...
    for (int first_boid{}; first_boid != this->size(); ++first_boid) {
      ...
      for (int second_boid{first_boid + 1}; second_boid != this->size();
           ++second_boid) {
    ...
    double mean_distance =
        mean_partial_sum / ((N * (N - 1)) / 2);  // compute mean
    for (int i{}; i != this->size(); ++i) {
      rms_partial_sum +=
          (dist_holder[i] - mean_distance) *
          (dist_holder[i] -
           mean_distance);  // sum of the squared deviation from mean
    }
    double distance_RMS =
        rms_partial_sum / (((N * (N - 1)) / 2) - 1);  // compute RMS
    return Vector{mean_distance, sqrt(distance_RMS)};
  } else {
    return Vector{0.0, 0.0};
  }
}

// calculating mean velocity and its RMS for a given time
Vector Flock::get_speed_mean_RMS() const {
  ...
  if (this->size() > 1) {
    ...
    for (int i{}; i != this->size(); ++i) {
      rms_partial_sum +=
          (speed_holder[i] - mean_speed) *
          (speed_holder[i] -
           mean_speed);  // sum of the squared deviation from mean
    }
    double speed_RMS = rms_partial_sum / (this->size() - 1);  // compute
                                                              // RMS
    return Vector{mean_speed, sqrt(speed_RMS)};
  } else {
    if (this->size() == 0) {
      return Vector{0.0, 0.0};
    } else {
