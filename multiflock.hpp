...
class MultiFlock {
 ...
 public:
  ...
  void evolve(double, double, double, std::function<void(Flock&)>);
  // evolve the multiflock and apply to each flock a user-defined function
...
inline std::vector<Boid> get_other_neighbours(MultiFlock const& multiflocks,
  ...
  for (int i{}; i != multiflocks.size(); ++i) {
    auto boids_i =
        flocks[i].get_boids();  // get all the boid for iteration flock
    bool member = is_member(boids_i, boid);
    if (member == false) {
      // if selected boid is not a member of the iteration flock copy all
      // iteration flock boid to other_boid vector
      std::copy(boids_i.begin(), boids_i.end(),
                std::back_inserter(other_boids));
    }
    if (member == true) {
      // if selected boid is a member if the iteration flock get its vision
      // angle and options
  ...
  // create a utility flock with all the boids not in the same flock with the
  // selected boid the utility flock has the opt and vision angle of the
  // selected boid's flock in order to call get_neighbours_of
  Flock other_boids_flock{other_boids, boid_options, alpha};
  // get all the effective neighbours for the selected boid which aren't in its
  // same flock
  other_neighbours = get_neighbours_of(other_boids_flock, boid);
  return other_neighbours;
}

// get mean distance and RMS of every flock in the multiflock
...
void MultiFlock::evolve(double delta_t, double max_speed, double min_speed) {
  ...
  std::for_each(flocks_.begin(), flocks_.end(), [&](Flock& flock) {
    ...
    for (size_t boid{}; boid != boid_vec.size();
         ++boid) {  // for each boid in flock
      std::vector<Boid> neighbours{get_other_neighbours(
          flocks_, boid_vec[boid])};  // get neighbours not in same flock
      boid_vec[boid].velocity +=
          separation(flock.get_options(), copy_vec[boid],
                     neighbours);  // apply separation to those boids
    }
    flock = Flock(boid_vec, flock.get_options(), flock.get_alpha());
  });

  // evolve each flock in multiflock
  std::for_each(flocks_.begin(), flocks_.end(),
                [delta_t, max_speed, min_speed](Flock& flock) {
                  flock.evolve(delta_t, max_speed, min_speed);
                });
}

// std multiflock evolve with the application of a costraint to apply on flocks
// after evolution
void MultiFlock::evolve(double delta_t, double max_speed, double min_speed,
                        std::function<void(Flock&)> costraint_applier) {
  this->evolve(delta_t, max_speed, min_speed);
  std::for_each(flocks_.begin(), flocks_.end(),
                [&](Flock& flock) { costraint_applier(flock); });
}
