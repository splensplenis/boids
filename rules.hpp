...
inline std::vector<Boid> get_neighbours_of(Flock const& flock,
  ...
  /*for (double i{}; i != flock.size(); ++i) {
    if ((boids[i]) != boid &&
        distance(boid, boids[i]) <= boids_options.distance) {
      neighbours.push_back(boids[i]);
    }
  }
  return neighbours;*/
  std::copy_if(boids.begin(), boids.end(), std::back_inserter(neighbours), [&](Boid const& boid1) {
    return (boid1 != boid) && (distance(boid1, boid) <= boids_options.distance);
  });
  return neighbours;
}
...
inline std::vector<Boid> view_neighbours(Flock const& flock, Boid const& boid) {
  ...
  // if the boid is still, it can see every neighbour
  if (speed(boid) == 0.) {
    return neighbours;
  }
  // if it is moving, only some neighbours are seen
  for (double i{}; i != neighbours.size(); ++i) {
    ...
    // neighbours viewed are the neighbours which its position makes an angle
    // with the position of the considered boid smaller than the vision angle
    if (theta <= alpha) {
...
// the nearby boids apply on the considered boid a speed change proportional to
// their distance towards him
...
// from the mean of the nearby boids is subtracted the speed of the considered
// boid multiplied by a fixed factor
...
// at the considered boid position is subtracted the centre of mass position of
// the nearby boids multiplied by a fixed factor
...
/* if the boid goes too much slow or too much fast:
  ...
  Doing so the boid accelerates/decelerates smoothly if he goes above or beyond
  these limits */
